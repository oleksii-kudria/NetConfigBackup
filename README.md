...
- **Cisco:** перед зняттям конфігурації виконується `terminal length 0`, щоб уникнути `--More--`; при обмежених правах можливі WARN про невдале вимкнення paging, але бекап продовжується.
...
- **Cisco:** before `show running-config` the tool sends `terminal length 0` to prevent `--More--`; if the command is rejected due to permissions, a warning is logged and the backup proceeds.
